...
class CreateDescription(Description):
    ...
    def categorie_description(self, value):
        ...
        descriptions = [" is extremely ", " is very ", "is quite ", " is relatively ", " is quite ", " is very ", " is extremely "]
        return descriptions[sum(value > t for t in thresholds)]


    def get_description(self, indice):
        self.FA_df = st.session_state.FA_df.apply(zscore, nan_policy="omit")
        self.FA_df = self.FA_df.iloc[self.indice, :].to_frame().T
       
        
        text = ''
        for i in st.session_state.FA_component_dict.keys():
            print(self.FA_df[i].values[0])
            label = self.get_1_label(st.session_state.FA_component_dict[i]["label"])
            text += 'The entitity '
            text += self.categorie_description(self.FA_df[i].values[0]) + label + '.'
            if self.FA_df[i].values[0] > 1:
                print(st.session_state.FA_component_dict[i]["top"])
                text+= 'In particular, the entidity say that ' + st.session_state.FA_component_dict[i]["top"][0]
                
            elif self.FA_df[i].values[0] < 1:
                text+= 'In particular, the entidity say that ' + st.session_state.FA_component_dict[i]["bottom"][0]
            else: 
                pass
        return text
